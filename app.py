    ...
    if flask.request.method == "POST":
        try:
            if flask.request.files.get("image"):
                ...
                cartoon_image = wb_cartoonizer.cartoonize(image)
                ...
                return render_template(
                    "index_cartoonized.html", cartoonized_image=cartoonized_img_name
                )
            ...
            if flask.request.files.get("video"):
                ...
                if opts["run_local"]:
                    ...
                    data_uri = upload_blob(
                        "processed_videos_cartoonize",
                        modified_video_path,
                        filename,
                        content_type="video/mp4",
                        algo_unique_key="cartoonizeinput",
                    )
                    ...
                    cartoon_video_path = download_video(
                        "cartoonized_videos",
                        os.path.basename(response["output_uri"]),
                        os.path.join(
                            app.config["UPLOAD_FOLDER_VIDEOS"],
                ...
                os.system(
                    "ffmpeg -hide_banner -loglevel warning -i '{}' -i '{}' -codec copy -shortest '{}'".format(
                        os.path.abspath(cartoon_video_path),
                        os.path.abspath(audio_file_path),
                ...
                return render_template(
                    "index_cartoonized.html", cartoonized_video=final_cartoon_video_path
                )
